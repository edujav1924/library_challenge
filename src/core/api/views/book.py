...
class BookViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiParameter(
                type=int, name="total_books", description="Total number of books"
            )
        },
    )
    ...
    def stats(self, request):
        ...
        data = Book.objects.aggregate(total_books=Count("id"))
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="search",
                description="Search for books by title, author name, or publication year.",
                required=False,
                type=str,
            ),
        ],
